...
public class MainServiceImpl implements MainService {
	...
	public int brandReqCount(int article_num, String email) {
		...
		if(itemVO.getDress()==null&&itemVO.getTshirt()==null&&itemVO.getPants()==null&&itemVO.getJacket()==null&&itemVO.getSneakers()==null&&itemVO.getSocks()==null)
			...
			if(gender==1 || gender==2){
				if(itemVO.getJacket()==null) itemVO.setJacket("");
				if(itemVO.getTshirt()==null) itemVO.setTshirt("");
				if(itemVO.getPants()==null) itemVO.setPants("");
				if(itemVO.getSneakers()==null) itemVO.setSneakers("");
				if(itemVO.getSocks()==null) itemVO.setSocks("");
				itemVO.setDress("");
			}else{
				if(itemVO.getJacket()==null) itemVO.setJacket("");
				if(itemVO.getTshirt()==null) itemVO.setTshirt("");
				if(itemVO.getPants()==null) itemVO.setPants("");
				if(itemVO.getDress()==null) itemVO.setDress("");
				if(itemVO.getSneakers()==null) itemVO.setSneakers("");
				if(itemVO.getSocks()==null) itemVO.setSocks("");
			}	
			hm = new HashMap<>();
	...
	@Override
	public void writeArticle(MultipartHttpServletRequest mreq,
		HttpServletRequest req, ArticleVO articleVO, ItemVO itemVO) {
	...
	@Override
	public void writeArticle(MultipartHttpServletRequest mreq,
			HttpServletRequest req, ArticleVO articleVO) {
		articleVO.setEmail(req.getSession().getAttribute("email").toString());
		articleVO.setName(req.getSession().getAttribute("name").toString());
		maindao.writeArticle(articleVO);
