...
<br>
 ...
 <c:if test="${empty imageList}">
      ...
      <c:if test="${imageList!=null}">
         <c:forEach var="content" items="${imageList}">
            <input type="hidden" id="image_num" value="${content.image_num}">
            <table align="center" border="2" Bordercolor=#1DE9C0 style="border-style:outset;border-color:#1DE9C0; float:left; margin:0.5em; z-index:15; BORDER-COLLAPSE:collapse">
               <tr>
                  <td align="center">
                  <a style="cursor: pointer;" class="btn-example" onclick="popupcontent(${content.article_num},${content.scrab_num})">
                  <img src="/closet/contentImageDown.main?image_num=${content.image_num}" id="main_img">
                  </a>
                  </td>
               </tr>
               ...
               <tr>
                  <td>
                     	<input type="button" value="좋아요 취소" onclick="deletelikeImage(${content.image_num},'${content.email}')" style="float: right">
                  </td>
               </tr>
            </table>
